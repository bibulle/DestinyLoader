            ...
            passport.authenticate('jwt-check', {session: false}, (err, user): any => {
                ...
                DestinyDb.readConf(user, function (err, conf) {
                    if (err) {
                        ...
                        let result = {
                            data: _.pick(conf, ['showOnlyPowerfulGear', 'language', 'selectedPursuits', 'user', 'visible', 'selectedTags']),
                            refreshedToken: user.refreshedToken,
                            version: {
                                version: Config.package_version,
                                commit: Config.package_commit
            ...
            passport.authenticate('jwt-check', {session: false}, (err, user): any => {
                ...
                DestinyDb.insertConf(user, conf, function (err, doc) {
                    if (err) {
                        ...
                        response.send(JSON.stringify(_.pick(doc, ['showOnlyPowerfulGear', 'language', 'selectedPursuits', 'user', 'selectedTags'], null, 2)));
                    }
                });
...
//          )
    ...
    router.route('/tag')
    // ====================================
    // route for setting tags on pursuits
    // ====================================
        .post((request: Request, response: Response, next: NextFunction) => {
            debug("POST /tag");
            ...
            passport.authenticate('jwt-check', {session: false}, (err, user): any => {
                ...
                const pursuitTags: PursuitTags = new PursuitTags(key, tags);
                DestinyDb.insertTag(pursuitTags, (err, result) => {
                    if (err) {
                        error(JSON.stringify(err, null, 2));
                        response.status(500).send({error: err});
                    } else {
                        response.send(JSON.stringify(pursuitTags, null, 2));
                    }
                })
