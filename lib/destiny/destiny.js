    ...
    async.eachSeries(
      ...
      function (item, callback) {
        async.waterfall([
            ...
            function (callback) {
              ...
              if (item.itemInstanceId) {
                if (itemInstancesTable[item.itemInstanceId]) {
                  ...
                  logger.warn(JSON.stringify(bucket, null, 2));
                }
              } else {
            ...
            function (item, callback) {
              ...
              try {
                newItem = {
                  ...
                  "bucketNameTarget": item.item.inventory.bucketTypeHash,
          ...
          function (err, item) {
            if (!result.items[item.bucketName]) {
              result.items[item.bucketName] = [];
            }
            result.items[item.bucketName].push(item);
            callback(err, result)
          });
    ...
    try {
      ...
      db.serialize(function () {
        ...
        db.each(query, function (err, row) {
          ...
          itemHashCacheByName[data.displayProperties.name] = data;
        }, function (err, cpt) {
          logger.info(cpt + " item definitions read");
