          ...
          async.eachSeries(
            ...
            function (item, callback) {
              async.waterfall([
                  ...
                  function (item, callback) {
                    //if ((item.item.displayProperties.name == "Uriel's Gift") || (item.item.displayProperties.name == "Traveler's Chosen")) {
...
var moveItem = function (user, item, itemCanBeEquipped, characterId, moveToVault, callback) {
  ...
  logger.info(JSON.stringify(item, null, 2));
  logger.info(JSON.stringify(itemCanBeEquipped, null, 2));
