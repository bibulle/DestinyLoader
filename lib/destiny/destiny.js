    ...
    async.eachSeries(
      ...
      function (item, callback) {
        async.waterfall([
            ...
            function (item, callback) {
              logger.info("------------ 1 "+item.itemInstanceId);
              //logger.info(JSON.stringify(item, null, 2));
              if (item.itemInstanceId) {
                if (itemSocketsTable[item.itemInstanceId]) {
                    ...
                    function(socket, callback) {
                      ...
                      if (socket.plugHash && socket.isEnabled) {
                        queryItemById(socket.plugHash, function(err, plug) {
                          ...
                          logger.info("============ 2.1 "+item.itemInstanceId);
                          return callback(null, item);

                        });
                      } else {
                        logger.info("============ 2.2 "+item.itemInstanceId);
                        return callback(null, item);
                      }
                    },
                    function(err) {
                      logger.info("============ 1.1 "+item.itemInstanceId);
                      return callback(err, item);
                    }
                  );
                  ...
                  //logger.warn(JSON.stringify(item, null, 2));
                  logger.info("============ 1.2 "+item.itemInstanceId);
                  callback(null, item);
                }
              } else {
                //logger.error("No item instance id");
                //logger.info(JSON.stringify(item, null, 2));
                logger.info("============ 1.3 "+item.itemInstanceId);
                callback(null, item);
              }
            },
            ...
            function (item, callback) {
              //if ((item.item.displayProperties.name == "Uriel's Gift") || (item.item.displayProperties.name == "Legion-Bane")) {
