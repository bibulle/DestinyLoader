...
var lockItem = function (user, item, state, callback) {
  ...
  _postFromBungie(url, {state: state, itemId: item.itemInstanceId, characterId: item.characterId, membershipType: user.destinyMemberships[0].membershipType}, function(err, data) {
    //logger.info(JSON.stringify(err, null, 2));
    //logger.info(JSON.stringify(data, null, 2));
    if (err) {
      return callback(err);
